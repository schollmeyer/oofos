...
test_that("compute_widths works", {
  ...
  widths1 <- compute_widths(betweenness >= stats::quantile(as.vector(betweenness),0.8))
  widths2 <- compute_widths(betweenness >= stats::quantile(as.vector(betweenness),0.7))
